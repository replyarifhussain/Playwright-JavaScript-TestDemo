...
import fs from 'fs'
import { username, password, loginButton } from '../pageobjects/homePage'
import {
	twitterLink,
	facebookLink,
	linkedInLink
...
test.describe.parallel(
	'@Sanity: Land as a standard user to verify the home page',
	() => {
		test('Land on Home Page as a standard user', async ({
			homePage,
		}) => {
			await test.step(`Open the APP and check base URL`, async () => {
				await homePage.openApp()
				expect(await homePage.getUrl()).toContain(baseUrl)
			})
			await test.step(`Open the APP and check title`, async () => {
				await homePage.openApp()
